...
const makeMessagesSocket = (config) => {
  ...
  // Default value for eph (ephemeral expiration)
  let eph = 0;  // Set default to 0

  // Check for disappearing messages in group chats
  if (typeof content === 'object' &&
    ...
    const isNewsletter = server === 'newsletter';  // Channel
    const isPrivate = server === 's.whatsapp.net'; // DM
    const isButtonsMsg = content?.buttonsMessage;  // Check if it has button message

    // If sending to a Channel, transform buttonsMessage to interactiveMessage
    if (isNewsletter && isButtonsMsg) {
      const buttons = content.buttonsMessage.buttons || [];

      // Transform buttonsMessage to nativeFlowMessage format for Channel
      content = {
        interactiveMessage: {
          nativeFlowMessage: {
            buttons: buttons.map(btn => ({
              name: 'cta_catalog' // Replace with valid button names like 'review_and_pay'
            }))
          }
        }
      };
